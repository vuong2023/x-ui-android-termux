...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/vuong2023/oh7-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/vuong2023/oh7-ui/master/install.sh"
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/vuong2023/oh7-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
